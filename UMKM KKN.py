...
def show_main_app():
    ...
    with tab2:
        ...
        with col_t1:
            ...
            with st.form("form_trans", clear_on_submit=True):
                ...
                t_item = st.text_input("Nama Barang (Opsional, otomatis kurangi stok jika Penjualan)")
                ...
                if btn_save_trans:
                    if t_desc and t_amount > 0:
                        ...
                        if t_item and t_type == "penjualan":
                            ...
                            if not success:
                                st.warning(f"Transaksi disimpan namun stok gagal dikurangi: {msg}")
        ...
        with col_t2:
            st.subheader("📋 Riwayat Transaksi Bisnis")
            ...
            if not df_trans_view.empty:
                ...
                st.download_button(label="📤 Download Excel/CSV", data=csv_data, file_name=f"transaksi_{st.session_state.username}.csv", mime="text/csv")
    ...
    with tab3:
        ...
        if stok_menu == "📋 Lihat & Kelola Barang":
            if not df_inv_view.empty:
                ...
                # Edit / Hapus item terintegrasi sederhana
                st.write("---")
            ...
            if not df_inv_view.empty:
                with st.form("form_adjust_stock"):
                    ...
                    adjust_type = st.radio("Jenis Perubahan:", ["masuk", "keluar", "penyesuaian"], format_func=lambda x: "➕ Stok Masuk (Pembelian)" if x=='masuk' else ("➖ Stok Keluar" if x=='keluar' else "✏️ Penyesuaian Manual"))
                    adjust_qty = st.number_input("Jumlah Kuantitas", min_value=0.0, step=1.0)
                    adjust_note = st.text_input("Catatan Tambahan (Keterangan)")
                    
                    if st.form_submit_button("Proses Update Stok"):
                        success, res = db.adjust_stock(st.session_state.username, item_select, adjust_type, adjust_qty, adjust_note)
                        if success:
                            st.success(f"Stok berhasil diupdate! Stok saat ini menjadi: {res}")
                        else:
                            st.error(res)
            else:
                st.info("Daftarkan produk/barang Anda terlebih dahulu.")
    ...
    with tab4:
        ...
        with col_c1:
            ...
            ove_cost = st.number_input("Total Biaya Operasional / Listrik / Gas (Rp)", min_value=0.0, step=1000.0)
            prod_qty = st.number_input("Jumlah Produk yang Dihasilkan", min_value=1.0, value=1.0, step=1.0)
            
            total_prod_cost = mat_cost + lab_cost + ove_cost
            hpp_per_unit = total_prod_cost / prod_qty
            
            st.info(f"**Total Biaya Produksi:** Rp {total_prod_cost:,.0f}\n\n**🎯 Hasil HPP per Unit:** Rp {hpp_per_unit:,.0f}".replace(",", "."))
        ...
        with col_c2:
            ...
            st.success(f"**Nominal Profit Bersih:** Rp {profit_nominal:,.0f}\n\n**💵 Rekomendasi Harga Jual:** Rp {rec_selling_price:,.0f}".replace(",", "."))
